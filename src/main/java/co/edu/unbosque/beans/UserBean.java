...
public class UserBean {
	...
	public String crear() {
		...
		if (adminUser.isValidPassword(user_password) == true) {
			userDto = new UserDTO(id, nombre, apellido, email, adminUser.encryptPassword(user_password), administrador,
					false);
	...
	public void login() {
		...
		// En este punto, el inicio de sesión no fue exitoso, así que se establecerá el
		// mensaje de acuerdo a la lógica
		if (mensajeLogin.equals("La cuenta está bloqueada.")
				|| mensajeLogin.equals("Se ha excedido el límite de intentos fallidos. La cuenta ha sido bloqueada.")) {
			mensaje = "La cuenta se encuentra bloqueada";
		} else if (mensajeLogin.equals("Credenciales incorrectas.")
				|| mensajeLogin.startsWith("Inicio de sesión fallido.")) {
