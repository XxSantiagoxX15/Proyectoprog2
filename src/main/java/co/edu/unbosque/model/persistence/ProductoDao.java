...
public class ProductoDao implements DaoCrud {
	...
	public boolean delete(int id) {
		...
		try {
			...
			em.createQuery("DELETE FROM Compra_inventario c WHERE c.producto = :producto").setParameter("producto", id)
					.executeUpdate();
			em.createQuery("DELETE FROM Detalle_factura d WHERE d.producto = :producto").setParameter("producto", id)
					.executeUpdate();
	...
	public ArrayList<Producto> findByInventory() {
		open();
		try {
			return (ArrayList<Producto>) em
					.createQuery(
							"SELECT p FROM Producto p WHERE p.cantidad_inventario <= 10 ORDER BY p.cantidad_inventario")
					.getResultList();
	...
	public List<Object[]> findBybestseller() {
		...
		return null; // Se recomienda usar List en lugar de ArrayList en la declaración del tipo de
						// retorno
	}
